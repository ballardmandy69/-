...
chmod +x /usr/local/bin/push_node_y1.sh

cat > /etc/systemd/system/nodecenter-node_y1.service << 'EOF'
[Unit]
Description=NodeCenter Push Service for node_y1
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/usr/local/bin/push_node_y1.sh
Restart=always
RestartSec=3
User=root
...
systemctl enable nodecenter-node_y1.service
systemctl restart nodecenter-node_y1.service
systemctl status nodecenter-node_y1.service --no-pager
