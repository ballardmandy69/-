...
#!/bin/bash
# AWS node_hk2 + node_hk26 install script.


cat > /usr/local/bin/push_node_hk2.sh << 'EOF'
#!/bin/bash

API_URL="https://nodecenter.hiccupc.xyz/push"
TOKEN="hiccupcc"
NODE_NAME_V4="node_hk2"
NODE_NAME_V6="node_hk26"
...
CHANGE_COOLDOWN=90
LAST_CHANGE_FILE_V4="/tmp/node_hk2_last_change_ip"
LAST_CHANGE_FILE_V6="/tmp/node_hk2_last_change_ipv6"
LOG_FILE="/var/log/nodecenter_node_hk2.log"
...
change_ipv4() {
  ...
  if [ -z "$NODE_ID" ]; then
    log "node_hk2 change ip skipped: NODE_ID empty"
    return 1
  fi
  ...
  if [ -z "$REGION_NAME" ]; then
    log "node_hk2 change ip skipped: region not found for instance=${NODE_ID}"
    return 1
  fi
  ...
  log "node_hk2 change ipv4... instance=${NODE_ID} region=${REGION_NAME}"
  ...
  log "node_hk2 ipv4 change resp=${IPV4_RESP}"
...
change_ipv6() {
  ...
  if [ -z "$NODE_ID" ]; then
    log "node_hk2 change ipv6 skipped: NODE_ID empty"
    return 1
  fi
  ...
  if [ -z "$REGION_NAME" ]; then
    log "node_hk2 change ipv6 skipped: region not found for instance=${NODE_ID}"
    return 1
  fi
  ...
  log "node_hk2 change ipv6... instance=${NODE_ID} region=${REGION_NAME}"
  ...
  log "node_hk2 ipv6 allocate resp=${IPV6_RESP}"
...
while true; do
  ...
  if check_ping_v4; then
    ...
    log "node_hk2 v4 ping failed count=${V4_FAIL_COUNT}/${FAIL_THRESHOLD_V4}"
  ...
  if check_ping_v6; then
    ...
    log "node_hk2 v6 ping failed count=${V6_FAIL_COUNT}/${FAIL_THRESHOLD_V6}"
...
chmod +x /usr/local/bin/push_node_hk2.sh

cat > /etc/systemd/system/nodecenter-node_hk2.service << 'EOF'
[Unit]
Description=NodeCenter Push Service for node_hk2 and node_hk26
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/usr/local/bin/push_node_hk2.sh
Restart=always
RestartSec=3
User=root
...
systemctl enable nodecenter-node_hk2.service
systemctl restart nodecenter-node_hk2.service
systemctl status nodecenter-node_hk2.service --no-pager -l
