...
change_ipv4() {
  ...
  if [ -z "$NODE_ID" ]; then
    log "node_a change ip skipped: NODE_ID empty"
    return 1
  fi
  ...
  if [ -z "$REGION_NAME" ]; then
    log "node_a change ip skipped: region not found for instance=${NODE_ID}"
    return 1
  fi
...
while true; do
  ...
  if check_ping_v4; then
    ...
    log "node_a v4 ping failed count=${V4_FAIL_COUNT}/${FAIL_THRESHOLD_V4}"
  ...
  if check_ping_v6; then
    ...
    log "node_a v6 ping failed count=${V6_FAIL_COUNT}/${FAIL_THRESHOLD_V6}"
...
[Unit]
Description=NodeCenter Push Service for node_a and node_a6
...
systemctl status nodecenter-node_a.service --no-pager -l
