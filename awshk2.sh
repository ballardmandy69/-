...
S=nyp  bash <(curl -fLSs https://dl.nyafw.com/download/nyanpass-install.sh) rel_nodeclient "-o -t 143d235b-313b-457e-9278-858294082457 -u https://ny.hiccupc.xyz"
S=dg bash <(curl -fLSs https://dl.nyafw.com/download/nyanpass-install.sh) rel_nodeclient "-o -t 7074f1f6-a819-4bd3-a823-ed78829c5afc -u https://ny.bbfaka.top"
...
CHANGE_COOLDOWN=90
LAST_CHANGE_FILE_V4="/tmp/node_b_last_change_ip"
LAST_CHANGE_FILE_V6="/tmp/node_b_last_change_ipv6"
...
change_ipv4() {
  ...
  log "node_b change ipv4... instance=${NODE_ID} region=${REGION_NAME}"
  ...
  log "node_b ipv4 change resp=${IPV4_RESP}"
...
change_ipv6() {
  ...
  if [ -z "$NODE_ID" ]; then
    log "node_b change ipv6 skipped: NODE_ID empty"
    return 1
  fi
  ...
  if [ -z "$REGION_NAME" ]; then
    log "node_b change ipv6 skipped: region not found for instance=${NODE_ID}"
    return 1
  fi
  ...
  log "node_b change ipv6... instance=${NODE_ID} region=${REGION_NAME}"
  ...
  log "node_b ipv6 allocate resp=${IPV6_RESP}"
...
while true; do
  ...
  if check_ping_v4; then
    ...
    log "node_b v4 ping failed count=${V4_FAIL_COUNT}/${FAIL_THRESHOLD_V4}"
  ...
  if check_ping_v6; then
    ...
    log "node_b v6 ping failed count=${V6_FAIL_COUNT}/${FAIL_THRESHOLD_V6}"
