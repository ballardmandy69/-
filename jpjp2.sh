...
if [[ -f /root/jpddns2.sh ]]; then
  chmod +x /root/jpddns2.sh
  echo "执行 /root/jpddns2.sh ..." | tee -a /root/ddns.log
  bash /root/jpddns2.sh >> /root/ddns.log 2>&1
  echo "✅ jpddns2.sh 执行完毕" | tee -a /root/ddns.log
else
  echo "❌ 下载 jpddns2.sh 失败，未找到 /root/jpddns2.sh" | tee -a /root/ddns.log
  exit 1
fi
