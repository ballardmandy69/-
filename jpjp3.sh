...
sysctl -w net.ipv4.tcp_pacing_ca_ratio=130
sysctl -w net.core.netdev_budget=3000
tc qdisc replace dev ens5 root fq
tc qdisc del dev ens5 root
tc -s qdisc show dev ens5

bash <(curl -fLSs https://file.hiccupc.xyz/hy2/sb-auto.sh) "json" "jpv3"
echo "开始下载 jpddns3.sh 到 /root 目录..." | tee -a /root/ddns.log
curl -fLSs https://file.hiccupc.xyz/hy2/jpddns3.sh -o /root/jpddns3.sh 2>> /root/ddns.log

if [[ -f /root/jpddns3.sh ]]; then
  chmod +x /root/jpddns3.sh
  echo "执行 /root/jpddns3.sh ..." | tee -a /root/ddns.log
  bash /root/jpddns3.sh >> /root/ddns.log 2>&1
  echo "✅ jpddns3.sh 执行完毕" | tee -a /root/ddns.log
else
  echo "❌ 下载 jpddns3.sh 失败，未找到 /root/jpddns3.sh" | tee -a /root/ddns.log
  exit 1
fi
