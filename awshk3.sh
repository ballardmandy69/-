...
CHANGE_COOLDOWN=90
LAST_CHANGE_FILE_V4="/tmp/node_my_last_change_ip"
LAST_CHANGE_FILE_V6="/tmp/node_my_last_change_ipv6"
...
change_ipv4() {
  ...
  log "node_my change ipv4... instance=${NODE_ID} region=${REGION_NAME}"
  ...
  log "node_my ipv4 change resp=${IPV4_RESP}"
...
change_ipv6() {
  ...
  if [ -z "$NODE_ID" ]; then
    log "node_my change ipv6 skipped: NODE_ID empty"
    return 1
  fi
  ...
  if [ -z "$REGION_NAME" ]; then
    log "node_my change ipv6 skipped: region not found for instance=${NODE_ID}"
    return 1
  fi
  ...
  log "node_my change ipv6... instance=${NODE_ID} region=${REGION_NAME}"
  ...
  log "node_my ipv6 allocate resp=${IPV6_RESP}"
