...
LAST_CHANGE_FILE_V6="/tmp/node_hk_last_change_ipv6"
...
change_ipv4() {
  ...
  log "node_hk change ipv4... instance=${NODE_ID} region=${REGION_NAME}"
...
change_ipv6() {
  ...
  if [ -z "$NODE_ID" ]; then
    log "node_hk change ipv6 skipped: NODE_ID empty"
    return 1
  fi
  ...
  if [ -z "$REGION_NAME" ]; then
    log "node_hk change ipv6 skipped: region not found for instance=${NODE_ID}"
    return 1
  fi
  ...
  log "node_hk change ipv6... instance=${NODE_ID} region=${REGION_NAME}"
